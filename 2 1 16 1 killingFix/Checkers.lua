...
--move it over
pieces[index].x = widthB - widthB/8/2 - widthB/8/2 * hostDeadCount;
pieces[index].y = 0 - widthB / 8;
hostDeadCount = hostDeadCount + 1;
else--is team guest
pieces[index].x = 0 + widthB / 8 / 2 + widthB / 8 / 2 * guestDeadCount;
...
local thisPiece = isSpaceOccupied(r1,c1);
...
tx:SetTexture('Interface/AddOns/Checkers/images/ka.tga');
else
tx:SetTexture('Interface/AddOns/Checkers/images/kh.tga');
end--end team switch
...
isValid, takenIndex = isValidMove(pieces[i].row,pieces[i].column,
...
tx:SetAlpha(1);
if (pieces[i].team == TEAM_HOST) then
tx:SetTexture('Interface/AddOns/Checkers/images/alliance_checker.tga');
else
tx:SetTexture('Interface/AddOns/Checkers/images/horde_checker.tga');
end
