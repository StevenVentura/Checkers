...
king = false;
alive = true;
lastX = convertColumnToX(c);
lastY = convertRowToY(r);
...
if (landedRow <= 0 or landedRow >= 9 or landedColumn <= 0 or landedColumn >= 9) then
pieces[i].checkerFrame:ClearAllPoints();
pieces[i].checkerFrame:SetPoint("BOTTOMLEFT",pieces[i].lastX,pieces[i].lastY);
